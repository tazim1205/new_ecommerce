                    ...
                    <form method="post" action="{{route('shipping.update',$shipping->id)}}" >
                        ...
                        <div class="col-lg-4 col-md-6 col-12" id="inputSingleBox">
                                ...
                                <select class="form-control" name="division_id" id="division_id" onchange="return GetDistrict()" required>
                                    ...
                                    <option value="{{$v->id}}" @if($div_info->id == $v->id) selected @endif >{{$v->division_name}}</option>
                                    @endforeach
                                    @endif
                                </select>
                                </div>
                            ...
                            <div class="col-lg-4 col-md-6 col-12" id="inputSingleBox">
                                ...
                                @if($dis_info)
                                    <option value="{{$dis_info->id}}" selected>{{$dis_info->district_name}} </option>
                                @else
                                    <option value="">@lang('common.select_one') </option>
                                @endif
                                </select>
                            ...
                            <div class="col-lg-4 col-md-6 col-12" id="inputSingleBox">
                                ...
                                @if($upazila_info)
                                    <option value="{{$upazila_info->id}}" selected>{{$upazila_info->upazila_name}} </option>
                                @else
                                    <option value="">@lang('common.select_one') </option>
                                @endif
                            ...
                            <label for="defaultFormControlInput" class="form-label">@lang('common.status')</label>
                                ...
                                <div class="form-check form-check">
                                    ...
                                    id="inactive"
                                    value="0"
                                    @if($shipping->status == 0) checked @endif
                                    />
        ...
        if(division_id != "")
        {
            $.ajax({
                ...
                success : function(data)
                {
                     ...
                     $('#upazila_id').html('<option value="">Select one</option>');
                }
            })
        }
        else{
